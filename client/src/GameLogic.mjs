...
class BoardModel {
  ...
  getPosition(piece, coord) {
    for (let row of range(0, 8)) {
      for (let col of range(0, 12)) {
        if (
          this.board[row][col] &&
          this.board[row][col].id === piece.id
        ) {
          return coord === 'row'
            ? row
            : coord === 'col'
        ...
        for (let row of range(oldRow + 1, newRow + 1)) {
          if (this.board[row][col])
            throw new CustomError(
              `path blocked at ${String.fromCharCode(
                col + 65
              )}${row.toString()}`,
              1
            )
  ...
  checkCustody(square, defender, attacker, suicide = false) {
    ...
    // corner cases
      ...
      if (square.attackVector === 'top') {
        this.custodialCapture(square, defender, attacker, {
          col: square.col + 1
        })
  ...
  removePiece(defender) {
    this.pieces = this.pieces.filter(
      (piece) => piece.id !== defender.id
    )
    this.board.forEach((row) => {
      const idx = row.findIndex((piece) => piece && piece.id === defender.id)
      if (idx >= 0) {
        row.splice(idx, 1, null)
      }
    })
  }
  // not used currently
