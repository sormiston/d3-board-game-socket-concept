...
class BoardView {
  ...
  constructor(game, parent) {
    ...
    socket.on('remoteDragStart', (payload) => {
      utils.dragstarted.call(
        this,
        { ...payload.event, remote: true },
        socket
      )
    })
  ...
  updateToPlayDisplay() {
    this.game.toPlay =
      this.game.toPlay.toLowerCase() === 'white' ? 'Black' : 'White'
    this.toPlayDisplay.innerText = `${this.game.toPlay} to move`
  }

  async drawGrid() {
    const parser = new DOMParser()
    const rootSvgData = parser.parseFromString(
      rootSvgSrc,
      'image/svg+xml'
    )
...
socket.on('connect', () => {
  ...
  socket.on('initialize', (data) => {
    if (!initialized) {
      ...
      const boardModel = new GameLogic.BoardModel(
        gameState.board,
        gameState.pieces
      )
      window.boardView = new BoardView(boardModel, '#game-area')
      initialized = true
    }
  })
})
